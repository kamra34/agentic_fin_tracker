...
def get_categories(
    ...
    """Get all categories for the current user, optionally filtered by type (expense, income, saving)"""
    service = CategoryService(db)
    return service.get_categories(current_user.id, include_inactive, category_type)
...
# Subcategory routes - MUST come before /{category_id} routes to avoid path conflicts
...
def update_subcategory(
    ...
    service = CategoryService(db)
    updated = service.update_subcategory(subcategory_id, current_user.id, subcategory_update)
    if not updated:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Subcategory not found or name already exists in this category"
        )
    return updated
...
# Category routes with /{category_id} - MUST come after subcategory routes
